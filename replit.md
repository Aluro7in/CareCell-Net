...
- **Real-time Chat System (Complete)**: Patient–donor messaging via `conversations` and `messages` DB tables. API routes: `POST /api/chat/start` (create/get conversation), `GET /api/chat` (list conversations with partner + last message), `GET /api/chat/:id` (fetch thread), `POST /api/chat/send` (send message). Frontend `/chat` page: conversation list with avatar initials, chat thread with gradient message bubbles, 3s auto-poll, auto-scroll, Enter to send. Added "Chat" (MessageCircle) to bottom nav as 7th item.
- **Light Mode Fixes (Complete)**: Replaced all standalone `text-white` with `text-foreground` across `home.tsx`, `dashboard.tsx`, `donor-card.tsx`, `patient.tsx`, `profile.tsx` — headings, card text, stat numbers, input fields, location field. Preserved `text-white` only on colored gradient buttons/badges where white is needed for contrast. Replaced `bg-secondary text-white` tab buttons with `text-secondary-foreground` for proper theme-aware contrast.
