    ...
    <div className="p-5 flex flex-col h-full gap-6">
      ...
      <div className="flex bg-card/80 p-1.5 rounded-xl border border-border backdrop-blur-sm">
        <button 
          ...
          className={`flex-1 py-2.5 text-sm font-semibold rounded-lg transition-all ${tab === 'requests' ? 'bg-secondary text-secondary-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}
        >
          Requests ({requests?.length || 0})
        </button>
        <button 
          onClick={() => setTab('alerts')}
          className={`flex-1 py-2.5 text-sm font-semibold rounded-lg transition-all ${tab === 'alerts' ? 'bg-secondary text-secondary-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}
        >
          Alerts
        </button>
      </div>
                    ...
                    <div className="flex justify-between items-start mb-4 pl-2">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <h3 className="font-bold text-foreground text-lg">{req.patientName}</h3>
                      ...
                      <div className="w-11 h-11 rounded-full bg-secondary flex items-center justify-center border border-border font-bold text-secondary-foreground shadow-sm">
                        {req.bloodGroup}
                      </div>
                    </div>
                      ...
                      {req.status !== 'fulfilled' && req.status !== 'cancelled' && (
                        <div className="flex gap-2">
                          ...
                          <button 
                            ...
                            className="w-10 h-10 flex items-center justify-center bg-secondary text-muted-foreground hover:bg-secondary/80 hover:text-foreground rounded-xl transition-colors border border-border"
                          >
              ...
              {alertLoading ? (
                ...
                alerts?.map((alert) => (
                  <motion.div whileHover={{ scale: 0.98 }} key={alert.id} className={`p-4 rounded-[20px] border flex items-center gap-4 shadow-sm ${alert.urgency === 'critical' ? 'bg-primary/5 border-primary/20' : 'bg-card border-border'}`}>
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 ${alert.urgency === 'critical' ? 'bg-primary/20 text-primary shadow-[0_0_15px_rgba(239,68,68,0.2)]' : 'bg-secondary text-secondary-foreground'}`}>
                      <Radio className="w-5 h-5" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center mb-1">
                        <h4 className="font-semibold text-foreground text-sm truncate">To: {alert.donorName}</h4>
                        <span className="text-[10px] text-muted-foreground font-medium">{format(new Date(alert.sentAt), "HH:mm")}</span>
                      </div>
                      <p className="text-xs text-muted-foreground truncate">
                        {alert.patientName} • <span className="font-bold text-foreground">{alert.bloodGroup}</span> • {alert.distanceKm.toFixed(1)}km
                      </p>
                    </div>
                  </motion.div>
                ))
