    ...
    <div className="bg-card/80 backdrop-blur-md rounded-[24px] p-5 border border-border shadow-sm flex flex-col h-full relative overflow-hidden">
      ...
      <div className="flex items-center gap-4 mb-5">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="font-bold text-white truncate text-base leading-tight mb-1">{donor.name}</h3>
      ...
      <div className="flex gap-2 mt-auto">
        <a href={`tel:${donor.phone}`} className="flex-1 flex items-center justify-center gap-1.5 py-3 rounded-xl bg-secondary hover:bg-secondary/80 text-white text-xs font-semibold transition-colors border border-border">
        ...
        <motion.button 
          ...
          className={`flex-1 flex items-center justify-center gap-1.5 py-3 rounded-xl text-xs font-semibold transition-colors border ${donor.available ? 'border-border bg-background text-muted-foreground hover:text-white' : 'border-green-500/30 bg-green-500/10 text-green-500'}`}
        >
